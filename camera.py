...
class Camera:
    ...
    def __init__(self, control_cmd_location: str, image_type: Optional[str] = None,
        ...
        self.image_index = now.strftime("%Y%m%d%H%M%S")
    ...
    def command_camera(self, command: str, image_postfix: str = None) -> str:
        ...
        if image_postfix is None:
            image_postfix = self.image_index
        # Build image name
        image_name = self.collection_name + '_' + str(image_postfix) + self.image_type
        # Command Camera
        system(f'\"{self.control_cmd_location}\" /filename {self.save_folder}{image_name} {command}')
        # system(f'\"{self.control_cmd_location}\" /nodownload {command}')
    ...
    def capture_single_image(self, autofocus: bool = False, image_postfix: str = None) -> str:
        ...
        self.image_index = now.strftime("%Y%m%d%H%M%S")
        if autofocus:
            return self.command_camera('/capture', image_postfix)
        else:
            return self.command_camera('/capturenoaf', image_postfix)
